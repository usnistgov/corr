...
def query_basic(words, page, filtr, current_user):
    ...
    if "project" not in filtrs:
        raw.extend([u for u in ProjectModel.objects().order_by('+created_at') if all(w in str(u.extended()) for w in words) and (project.access == 'public' or current_user and (current_user == project.owner or current_user.group == "admin"))])

        pagination_logs.append("{0} -- query_basic: {1}".format(datetime.datetime.utcnow(), raw))
    if "record" not in filtrs:
        raw.extend([u for u in RecordModel.objects().order_by('+created_at') if all(w in str(u.extended()) for w in words) and (record.access == 'public' or current_user and (current_user == record.project.owner or current_user.group == "admin"))])
        pagination_logs.append("{0} -- query_basic: {1}".format(datetime.datetime.utcnow(), raw))
    if "diff" not in filtrs:
        raw.extend([u for u in DiffModel.objects().order_by('+created_at') if all(w in str(u.extended()) for w in words) and ((diff.record_from.access == 'public' and diff.record_to.access == 'public') or (current_user and (current_user.group == "admin" or current_user == diff.record_from.project.owner or current_user == diff.record_to.project.owner)))])
        pagination_logs.append("{0} -- query_basic: {1}".format(datetime.datetime.utcnow(), raw))
